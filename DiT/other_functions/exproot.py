...
class Exproot(nn.Module):
    ...
    def forward(self, x):
        ...
        x_transformed = torch.sign(x_transformed) * (1 - torch.exp(-torch.sqrt(torch.abs(x_transformed))))
...
def convert_ln_to_exproot(module, alpha_init_value=0.5, shift_init_value=0.0):
    module_output = module
    if isinstance(module, nn.LayerNorm):
        module_output = Exproot(module.normalized_shape, module.elementwise_affine, alpha_init_value=alpha_init_value, shift_init_value=shift_init_value)
    for name, child in module.named_children():
        module_output.add_module(name, convert_ln_to_exproot(child, alpha_init_value, shift_init_value))
    del module
