...
class Saturlog(nn.Module):
    ...
    def forward(self, x):
        ...
        log_term = torch.log(torch.abs(x_transformed) + 1)
        x_transformed = torch.sign(x_transformed) * log_term / (log_term + 1)
...
def convert_ln_to_saturlog(module, alpha_init_value=0.5, shift_init_value=0.0):
    module_output = module
    if isinstance(module, nn.LayerNorm):
        module_output = Saturlog(module.normalized_shape, module.elementwise_affine, alpha_init_value=alpha_init_value, shift_init_value=shift_init_value)
    for name, child in module.named_children():
        module_output.add_module(name, convert_ln_to_saturlog(child, alpha_init_value, shift_init_value))
    del module
