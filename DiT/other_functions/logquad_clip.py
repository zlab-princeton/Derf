...
class Logquad_clip(nn.Module):
    ...
    def forward(self, x):
        ...
        x_transformed = torch.clip(torch.sign(x_transformed) * torch.log(x_transformed**2 + 1), -1, 1)
...
def convert_ln_to_logquad_clip(module, alpha_init_value=0.5, shift_init_value=0.0):
    module_output = module
    if isinstance(module, nn.LayerNorm):
        module_output = Logquad_clip(module.normalized_shape, module.elementwise_affine, alpha_init_value=alpha_init_value, shift_init_value=shift_init_value)
    for name, child in module.named_children():
        module_output.add_module(name, convert_ln_to_logquad_clip(child, alpha_init_value, shift_init_value))
    del module
