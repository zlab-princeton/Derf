...
class Arctan(nn.Module):
    ...
    def forward(self, x):
        ...
        x_transformed = 2 / torch.pi * torch.atan(x_transformed)
...
def convert_ln_to_arctan(module, alpha_init_value=0.5, shift_init_value=0.0):
    module_output = module
    if isinstance(module, nn.LayerNorm):
        module_output = Arctan(module.normalized_shape, module.elementwise_affine, alpha_init_value=alpha_init_value, shift_init_value=shift_init_value)
    for name, child in module.named_children():
        module_output.add_module(name, convert_ln_to_arctan(child, alpha_init_value, shift_init_value))
    del module
