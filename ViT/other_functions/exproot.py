...
class Exproot(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sign(x) * (1 - torch.exp(-torch.sqrt(torch.abs(x))))
...
def convert_ln_to_exproot(module, alpha_init_value=0.5, shift_init_value=0.0):
    ...
    if isinstance(module, nn.LayerNorm):
        module_output = Exproot(module.normalized_shape, not isinstance(module, LayerNorm2d), alpha_init_value, shift_init_value)
